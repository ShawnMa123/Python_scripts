...
def calculate_dca_returns(ticker, start_date, end_date, weekly_investment, transaction_fee):
    ...
    including yearly and total returns, and plots the ticker's closing price on a secondary y-axis.
    ...
    # Plotting with ticker price on a secondary y-axis
    fig, ax1 = plt.subplots(figsize=(12, 8))

    ax1.plot(portfolio_df.index, portfolio_df['Portfolio Value'], label='Portfolio Value', color='blue')
    ax1.plot(portfolio_df.index, portfolio_df['Net Investment'], label='Net Investment', linestyle='--', color='green')
    ax1.set_xlabel('Date')
    ax1.set_ylabel('Portfolio/Investment Value (USD)', color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')

    ax2 = ax1.twinx()
    ax2.plot(data['Close'], label=f'{ticker} Close Price', alpha=0.7, linestyle=':', color='orange')
    ax2.set_ylabel(f'{ticker} Close Price (USD)', color='orange')
    ...
    for year, row in yearly_returns.iterrows():
        ax1.annotate(f"{row['YearlyReturnRate']:.1f}%",
